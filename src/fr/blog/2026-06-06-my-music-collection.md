...
2. **J'utilise une stratégie de nommage définie et constante.**
...
3. **Les rips sauvegardés sont comparés à la base de données [AccurateRip](https://www.accuraterip.com/).** 
...
L'idée est ainsi d'avoir une organisation des fichiers qui a du sens, et se reposer sur le système de tag intégré pour pouvoir retrouver un artiste / album / genre le plus facilement possible. Cela facilite l'écoute de certains albums aussi, comme par exemple [**NTS Sessions 1-4**](https://www.discogs.com/release/12398536-Autechre-NTS-Sessions) d'**Autechre**, qui a la particularité de durer 8 heures. Je suis donc dans la logique de 1 disque physique = 1 dossier.
...
Second logiciel le plus important après Fre:ac, [Picard](https://picard.musicbrainz.org/) permet d'inscrire les tags (les métadonnées des albums) depuis une base de données centralisée et normalisée, véritable encyclopédie universelle qui regroupe des millions de titres.
...
Bien évidemment, un tel travail mérite une solution de sauvegarde pérenne. Pour cela, j'ai opté pour une solution simple : toute ma collection est sauvegardée sur deux disques différents (inhérent à ma configuration actuelle où j'ai deux OS différents), ainsi qu'un disque dur externe. Enfin, pour palier à un éventuel problème lié au stockage et répondre à la [règle des 3-2-1](https://www.seagate.com/fr/fr/blog/what-is-a-3-2-1-backup-strategy/), j'utilise un [service de stockage « froid » de chez Scaleway](https://www.scaleway.com/fr/glacier-cold-storage/).

## Créer son propre Spotify

Malgré cette solution de sauvegarde sûre et éprouvée, je me suis vite rendu compte qu'elle n'était pas tenable sur le long terme. Devoir synchroniser deux bibliothèques (sur mon PC principal et mon laptop) est fastidieux. Je pourrais bien évidemment utiliser un outil comme [**Syncthing**](https://syncthing.net/), mais cela ne solutionnait pas un autre problème de taille : l'écoute en nomade.

J'ai donc pris la décision de louer un serveur et installer dessus [**Navidrome**](https://www.navidrome.org/). Je peux facilement y synchroniser l'intégralité de ma bibliothèque grâce à **rsync**, et j'ai accès à toute ma musique, où que je sois dans le monde.
